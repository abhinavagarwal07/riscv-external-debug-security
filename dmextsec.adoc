...
The NDMRESET operation is a system-level reset not tied to hart privilege levels and resets the entire system (excluding the Debug Module). The Debug Module Security Extension makes the NDMRESET field read-only 0 when <<nsecdbg, `nsecdbg`>> is 0. The debugger can determine support for the NDMRESET operation by setting the field to 1 and subsequently verifying the returned value upon reading.
...
==== Data and Program Buffer Registers

The `data` and `progbuf` registers may hold values obtained while an operation or state access was permitted by the external debug security controls and is no longer permitted because those controls changed, including after `nsecdbg` changes from 1 to 0. The Debug Module must invalidate any contents of these registers that may have been obtained in this way. Invalidated bits must not be readable over the Debug Module Interface, readable through a hart-visible alias or from Program Buffer memory, used as an operand of an abstract command, or executed from the Program Buffer, and an abstract command that started before the controls changed must not make them valid.

A write over the Debug Module Interface that takes effect makes valid only the bits it writes. An abstract command that was permitted and executed under the current external debug security controls and completes successfully makes valid only the bits it writes. Bits that are not written, or that are left UNSPECIFIED, remain invalid.

[NOTE]
Clearing these registers when the controls change is not sufficient on its own, because an abstract command issued earlier completes by writing its result into the `data` registers, and those registers may hold the operands of a command that is still executing.

=== System Bus Access 

The System Bus Access must be checked by bus initiator protection mechanisms such as IOPMP cite:[iopmp] or WorldGuard cite:[worldguard]. The bus protection unit can return an error to the Debug Module on illegal accesses; in that case, the Debug Module will set `sbcs`.SBERROR to 6 (security fault error).

When `nsecdbg` changes from 1 to 0, the Debug Module must invalidate the `sbdata` registers. Invalidated bits must not be readable over the Debug Module Interface or used by a subsequent system bus write, and a system bus access that started before the change must not make them valid. A write over the Debug Module Interface that takes effect makes valid only the bits it writes. A system bus read makes valid only the low bits corresponding to the access width selected by `sbcs`.SBACCESS; the remaining high bits remain invalid.
